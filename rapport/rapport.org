...
** Le domaine du calcul haute performance
   ...
   Enfin nous les donnons à résoudre à des unités de calcul différentes dans un
   ordre qui dépend de l'arbre de tâches construit. Un exemple de graphe de
...
** Sujet d'étude et plan
   ...
   dans l'équipe, peut aussi traiter des applications stencil efficacement.
...
** Ordonnancer un ensemble de tâches
   ...
   tâches est de savoir à qui demande-t-on de calculer une tâches parmi toutes
   les unités de calcul disponibles. Lorsque l'on souhaite n'utiliser que des
   CPUs, une solution consiste à laisser l'ordonnancement des tâches au système
   d'exploitation. Cette solution perd de son sens lorsque qu'on y ajoute un ou
   plusieurs GPUs. De plus, le système d'exploitation n'a pas conscience des
   autres machines reliées au réseau.

   Une autre solution consiste à laisser l'utilisateur assigner, une à une, les
   tâches à un ouvrier. Enfin, et c'est le sujet de ce stage, nous
...
** Architecture hétérogène
   ...
   unités de calcul des carte graphique. Ils sont généralement bien plus rapide
   ...
   Comme illustré figure [[fig:memarch]] chaque carte graphique dispose de sa propre
   mémoire. Celle-ci est cependant plus limitée en volume que la mémoire
   principale, soit quelques Go pour une carte graphique contre plusieurs
   dizaines de Go pour la mémoire principale. De plus, les unités de calcul
   d'une carte graphique ne peuvent pas directement avoir accès à la mémoire
   principale. Lorsque l'on souhaite accéder à une donnée, il faut la charger
   depuis la mémoire principale vers la mémoire de la carte graphique. La
   latence étant plus élevée qu'un accès par un CPU.
...
[[file:img/memarch.png]]
   ...
   réguler aux mieux les transferts vers les mémoires des GPUs. Pour cela, nous
   pouvons essayer de :
   - limiter le volume de données en réduisant les transferts au strict minimum
...
[[file:img/runtime.pdf]]
    ...
    ainsi décider de la répartition des tâches de calcul sur les unités de
    calcul à disposition. Pour prendre ces décision, il utilise des politiques
...
*** Solutions existantes au problème
    ...
    minimiser le nombre de défauts de cache (TODO: définir cette notion) sans
    connaître la taille du cache. Pour cela, on maximise la réutilisation des
    données . Cela nécessite aussi une charge de travail supplémentaire pour
    l'utilisateur.
...
** Résumé de la contribution
   ...
   Enfin, nous avons écrit un ordonnanceur simple adapté aux applications de
   type stencil.
...
*** /Eager/ (=eager= et =prio=)
    ...
    Cette ordonnanceur ne travail pas du tout sur la localité des données.
    Cependant, en utilisant cette ordonnanceur, l'application peut feindre de la
...
*** /Deque Model Data Aware/ (=dmda= et =dmdar=)
  ...
  =dmdar= est une variante de =dmda= qui trie les files des unités de calcul par
  quantité de données déjà disponibles dans la mémoire de l'ouvrier. Cela permet
  de maximiser le recouvrement des transferts par du calcul.
#+BEGIN_LATEX
\begin{figure}
\centering
\hspace*{\fill}
...
*** /Locality Work Stealing/ (=lws=)
    ...
    Un ouvrier pourra faire du vol travail s'il se retrouve inactif. Il pourra
...
*** /Modular-heft/
    ...
    mémoires est donc cruciaux ici.

    Lorsque la liste est vide, on va aller
    notifier le composant racine. Celui-ici va alors essayer de remplir la liste
    de l'ouvrier qui a fait la demande avec des tâches dont les données lui sont
    déjà accessibles.

    Le composant /mct/ (/Manufacturing Critical Time/) ...
...
** Méthode de référence : Algorithme cache oblivieux de soumission de tâches
    ...
    transférées. À priori, il devrait s'agit de la méthode idéale lorsque le
    problème étudié est trop grand pour rentrer en mémoire GPU.

    L'algorithme étant adapté pour une exécution séquentiel, il a fallu
    l'adapter pour une exécution parallèle. Pour cela, nous découpons le
...
** Outils de visualisation
   ...
   dispose déjà d'outil d'analyse de traces qui permettent d'observer si un
...
*** Diagramme d'exécution : domaine en fonction des itérations
    ...
    isochrones à la verticale s'est déroulé durant le même laps de
    temps.
#+BEGIN_LATEX
\begin{figure}
\centering
...
\hspace*{\fill}
\subfloat[Exécution avec deux GPUs.]
  {\includegraphics[width=0.15\linewidth]{img/example-iter.png}}
\hspace*{\fill}
\subfloat[Légende.]
  {\includegraphics[width=0.6\linewidth]{img/xpm-legend-iter.png}}
\hspace*{\fill}
...
*** Expérimentation simulées à l'aide de /SimGrid/
    ...
    stencil moins factice que celui utilisé durant le stage, ne sera pas faite
    durant le stage.

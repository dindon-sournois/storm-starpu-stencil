...
** Architecture hétérogène
   ...
   beaucoup utilisés en calcul haute performance pour faire du calcul généraliste.

   Comme illustré figure [[fig:memarch]] chaque carte graphique dispose de sa propre mémoire.
   Celle-ci est cependant plus limitée en volume que la mémoire principale, soit
   quelques Go pour une carte graphique contre plusieurs dizaines de Go pour la
...
*** Économiser de la bande passante
    ...
    traitant plusieurs itérations de manières successives, comme illustrée figure *???*.
